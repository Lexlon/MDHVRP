...
def arg_parser():
    ...
    parser.add_argument('--seed', metavar='SE', type=int, default=123,
                        help='random seed number for inference(or validation during training), reproducibility')
    # main config
    parser.add_argument('-n', '--n_customer', metavar='N', type=int, default=40,
                        help='number of customer nodes, time sequence')
    parser.add_argument('-c', '--n_car_each_depot', metavar='C', type=int, default=3,
                        help='number of available vehicles per depot')
    ...
    parser.add_argument('-s', '--batch_steps', metavar='S', type=int, default=1000,
                        help='number of samples = batch * batch_steps')
    parser.add_argument('-v', '--batch_verbose', metavar='V', type=int, default=1000,
    ...
    parser.add_argument('-nr', '--n_rollout_samples', metavar='NR', type=int, default=1000,
                        help='baseline rollout number of samples')
    parser.add_argument('-nv', '--n_val_samples', metavar='NV', type=int, default=1000,
                        help='validation number of samples during training')
    ...
    parser.add_argument('-th', '--tanh_clipping', metavar='TH', type=float, default=10.,
...
def test_parser():
    parser = argparse.ArgumentParser()
    parser.add_argument('-dt', '--decode_type', metavar='DT', type=str, default='greedy',
                        choices=['greedy', 'sampling'], help='greedy or sampling')
    parser.add_argument('-b', '--batch', metavar='B', type=int, default=2, help='batch size')
    parser.add_argument('-n', '--n_customer', metavar='N', type=int, default=50,
                        help='number of customer nodes, time sequence')
    ...
    parser.add_argument('-p', '--path', metavar='P', type=str, default='Weights/VRP20_epoch28.pt',
                        help='Weights/VRP***_epoch***.pt, pt file required')
    parser.add_argument('-t', '--txt', metavar='T', type=str, default='data/n20d3c3s1234.json',
    ...
    parser.add_argument('-wc2', '--write_csv_2opt', metavar='WC2', type=str,
                        help='export csv example: Csv/***_mha2opt.csv')
    args = parser.parse_args()
    return args
