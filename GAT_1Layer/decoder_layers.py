...
class DotProductAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        """ Q:(batch, self.n_heads, n_car, 1, self.head_depth)
            K:(batch, self.n_heads, n_car, n_nodes, self.head_depth)
            V:(batch, self.n_heads, n_car, n_nodes, self.head_depth)
            logits: (batch, self.n_heads, n_car, 1, n_nodes)
...
if __name__ == '__main__':
    ...
    batch, n_car, n_nodes, embed_dim = 5, 9, 23, 128
