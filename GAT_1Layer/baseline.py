...
def load_model(device, path, embed_dim=128, n_encode_layers=3):
    ...
    # 如果模型在DataParallel上保存，则删除"module."前缀
...
class RolloutBaseline:
    ...
    def __init__(self, model, task, weight_dir, n_rollout_samples=10000,
                 embed_dim=128, n_car_each_depot=3, n_depot=2, n_customer=20, capa=1., warmup_beta=0.8, wp_epochs=1,
                 device='cpu',
    ...
    def _update_baseline(self, model, epoch):
        ...
        if self.from_checkpoint and self.alpha == 0:
            ...
            self.model = load_model(self.device, self.path_to_checkpoint, embed_dim=self.embed_dim)
        else:
        ...
        self.dataset = Generator(self.device, n_samples=self.n_rollout_samples, n_car_each_depot=self.n_car_each_depot,
                                 n_depot=self.n_depot, n_customer=self.n_customer, seed=None)
    ...
    def epoch_callback(self, model, epoch, batch=64):
        ...
        candidate_vals = self.rollout(model, self.dataset,
                                      batch).cpu().numpy()  # costs for training model on baseline dataset
        candidate_mean = candidate_vals.mean()

        print(f'Epoch {self.cur_epoch} candidate mean {candidate_mean}, baseline mean {self.mean}')

        if candidate_mean < self.mean:
            print('candidate_vals.shape', candidate_vals.shape)
            print('self.bl_vals', self.bl_vals.shape)
            t, p = ttest_rel(candidate_vals, self.bl_vals[0:candidate_vals.shape[0]])  # scipy.stats.ttest_rel
    ...
    def rollout(self, model, dataset, batch=1000, disable_tqdm=False):
        arr = []
        dataloader = DataLoader(dataset, batch_size=batch, drop_last=False)
        model.eval()
        with torch.no_grad():
