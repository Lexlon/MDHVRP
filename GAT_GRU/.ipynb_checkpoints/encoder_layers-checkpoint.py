...
class DotProductAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            print('mask.size():', mask.size())
            print('logits.size():', logits.size())
            print('mask[:,None,:,:].squeeze(-1).repeat(1,logits.size(1),1,logits.size(-1):',
                  mask[:, None, :, :].squeeze(-1).repeat(1, logits.size(1), 1, mask.size(1)).size())
            # logits = logits.masked_fill(mask[:,None,None,:,0].repeat(1,logits.size(1),1,1) == True, -self.inf)
            logits = logits.masked_fill(
                mask[:, None, :, :].squeeze(-1).repeat(1, logits.size(1), 1, mask.size(1)) == True, -self.inf)
