...
def plot_route(tup, title):
    ...
    layout = go.Layout(
        ...
        title=dict(text=f'<b>VRP{customer_xy.shape[0]} depot{depot_xy.shape[0]} {title}, Total Length = {cost:.3f}</b>',
        ...
        xaxis=dict(title='X', range=[0, 1], linecolor='black', showgrid=False, ticks='outside', linewidth=1,
                   mirror=True),
        yaxis=dict(title='Y', range=[0, 1], linecolor='black', showgrid=False, ticks='outside', linewidth=1,
                   mirror=True),
        showlegend=True,
        width=750,
        height=700,
...
if __name__ == '__main__':
    ...
    if args.txt is not None:
        ...
        for k in ['depot_xy', 'customer_xy', 'demand', 'car_start_node', 'car_capacity']:
            elem = [generate_data(device, batch=1, n_car=args.n_car, n_depot=args.n_depot, n_customer=args.n_customer,
                                  seed=args.seed)[k].squeeze(0) for j in range(args.batch)]
