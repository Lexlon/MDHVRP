...
class GraphAttentionEncoder(nn.Module):
    def __init__(self, embed_dim=128, n_heads=8, n_layers=3, FF_hidden=512):
        ...
        self.init_W = nn.Linear(4, embed_dim, bias=True)
    ...
    def forward(self, x, mask=None):
        ...
        embed_customer = self.init_W(torch.cat([x['customer_xy'], x['demand_level'][:, :, None], x['demand'][:, :, None]], dim=-1))
...
if __name__ == '__main__':
    ...
    batch, n_car_each_depot, n_depot, n_customer, seed=10, 3, 3, 20, 1234
    encoder = GraphAttentionEncoder(n_layers=3)
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    encoder = encoder.to(device)
    data = generate_data(device, batch=batch, n_car_each_depot=n_car_each_depot, n_depot=n_depot, n_customer=n_customer, seed=seed)
    output = encoder(data, mask=None)
    print('output[0].shape:', output[0].size())
    print('output[1].shape', output[1].size())

    #summary(encoder, [(3,2), (20,2), (20,)])


    cnt = 0
    for i, k in encoder.state_dict().items():
        print(i, torch.numel(k))
        cnt += torch.numel(k)
    print(cnt)
